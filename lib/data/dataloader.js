...
function loadEntries(ddata, ctx, callback) {
    ...
    const longLoad = Math.round(constants.THREE_HOURS); //Math.round(constants.TWO_DAYS);
    ...
    ctx.entries.list(q, function(err, results) {
        ...
        if (!err && results) {
            ...
            const ageLimit = ddata.lastUpdated - constants.THREE_HOURS; // constants.TWO_DAYS;
            ddata.sgvs = sgvs;
            ddata.mbgs = mbgs;
            ddata.cals = cals;
        }
