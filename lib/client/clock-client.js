...
client.render = function render () {
  ...
  if (latestProperties.cage) {
    if (latestProperties.cage.level > 0) {
      xtraDisplayValue += " CAGE:" + latestProperties.cage.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " cage:" + latestProperties.cage.display;
    }
  ...
  if (latestProperties.sage && latestProperties.sage["Sensor Start"]) {
    if (latestProperties.sage["Sensor Start"].level > 0) {
      xtraAlarm = true;
      xtraDisplayValue += " SAGE:" + latestProperties.sage["Sensor Start"].display;
    } else {
      xtraDisplayValue += " sage:" + latestProperties.sage["Sensor Start"].display;
    }
  }
  if (latestProperties.iage) {
    if (latestProperties.iage.level > 0) {
      xtraDisplayValue += " IAGE:" + latestProperties.iage.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " iage:" + latestProperties.iage.display;
    }
  ...
  if (latestProperties.bage) {
    if (latestProperties.bage.level > 0) {
      xtraDisplayValue += " BAGE:" + latestProperties.bage.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " bage:" + latestProperties.bage.display;
    }
  ...
  if (latestProperties.upbat) {
    if (latestProperties.upbat.level <= 25) {
      xtraDisplayValue += " UP:" + latestProperties.upbat.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " up:" + latestProperties.upbat.display;
    }
  ...
  if (latestProperties.pump && latestProperties.pump.data) {
    if (latestProperties.pump.data.reservoir.level > 0) {
      xtraDisplayValue += " R:" + latestProperties.pump.data.reservoir.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " r:" + latestProperties.pump.data.reservoir.display;
    }
    if (latestProperties.pump.data.battery.level > 0) {
      xtraDisplayValue += " PB:" + latestProperties.pump.data.battery.display;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " pb:" + latestProperties.pump.data.battery.display;
    }
  ...
  if (latestProperties.sensorState) {
    if (latestProperties.sensorState.level > 0) {
      xtraDisplayValue += " CGM:" + latestProperties.sensorState.lastStateStringShort + " VB:" + ;latestProperties.sensorState.lastVoltageB;
      xtraAlarm = true;
    } else {
      xtraDisplayValue += " cgm:" + latestProperties.sensorState.lastStateStringShort + " vb:" + ;latestProperties.sensorState.lastVoltageB;
    }
  ...
  if (latestProperties.dbsize) {
    xtraDisplayValue += " db:" + latestProperties.dbsize.display;
  }
  if (latestProperties.iob) {
    xtraDisplayValue += " iob:" + latestProperties.iob.display;
  }
  if (latestProperties.basal) {
    xtraDisplayValue += " " + latestProperties.basal.display;
  }
  ...
  $('.xt').toggleClass('stale', xtraAlarm).html(xtraDisplayValue);
