...
client.render = function render () {
  ...
  let graphHTMLPre = "";
  let graphHTMLPost = "";
  if (latestProperties.buckets && graphWidth > 0) {
    // TBD
...
client.updateParams = function updateParams () {
  ...
  for (let param in faceParams) {
    ...
    // open weathermap params: -ow{fontsize}:{F|C}:zip:apikey 
    // example: -ow16:F:01108:D17C57239B5661055D459A608F770208
    //          (note: this is random, not a valid OWM key)
    ...
    // SGV graph size params: -gr{width:height}
    if (faceParam.substr(0, 2) === 'gr') {
      const graphSize = faceParam.substr(2).split(":");
