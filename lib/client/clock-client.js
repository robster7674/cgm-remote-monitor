...
client.pluginInfo = function pluginInfo() {
  ...
  if (latestProperties.sensorState) {
    if (latestProperties.sensorState.level > 0) {
      ...
      pluginisplayValue += " cgm:" + latestProperties.sensorState.lastStateStringShort.toLowerCase();
      if (latestProperties.sensorState.lastVoltageB !== undefined) {
        pluginDisplayValue += " vb:" + latestProperties.sensorState.lastVoltageB;
      }
    }
...
client.render = function render () {
  ...
  let pluginDisplayValue = client.pluginInfo();
  ...
  let graphHTMLPre = "";
  let graphHTMLPost = "";
  if (latestProperties.buckets && graphPixels > 0) {
    // TBD
    graphHTMLPre = '<div id="graph" style="width: 100%; height: 300px; border: 1px solid black; position: relative;"> \
                    ...
                    <div style="position: absolute; bottom: 12%; left: 80%; width: 30px; height: 30px; background-color: red; border-radius: 100%;"></div>';
    graphHTMLPost = "</div></div>";
  }
  ...
  let displayValue = graphHTMLPre + rec.scaled + graphHTMLPost;
...
client.init = function init () {
  ...
  for (let param in faceParams) {
    ...
    if (faceParam.substr(0, 2) === 'ow') {
      // example: -ow16:F:01108:D17C57239B5661055D459A608F770208
      // (this is random, not a valid OWM key)
      const owmParams = faceParam.substr(2).split(":");
      owmUnits = owmParams[1].toUpperCase();
      owmZip = owmParams[2].replace(/ /g,'%20');
      owmKey = owmParams[3];
    }
    // also get sgv graph size param
    if (faceParam.substr(0, 2) === 'gr') {
      graphPixels = faceParam.substr(2);
    }
  }
  if (owmKey !== "") {
    client.queryOWM();
