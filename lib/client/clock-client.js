...
client.query = function query () {
  ...
  var treatmentSrc = '/api/v2/treatments?find[eventType][$ne]=Temp+Basal';

  if (lastTreatmentDate) {
    treatmentSrc += "&find[created_at][$gt]=";
  } else {
...
function treatmentsDisplay() {
  ...
  if (treatmentData['Announcement']) {
    ...
    if (elapsedms < hour) {
      value += "Annc: " + treatmentData['Announcement'].notes + " " + Math.floor(elapsedms / 1000 / 60) + "min ago";
    }
  }
  if (treatmentData['Profile Switch']) {
    let elapsedms = now - Date.parse(treatmentData['Profile Switch'].created_at);
    if (elapsedms < hour) {
      value += "Prof: " + treatmentData['Profile Switch'].profile + " ";
      if (treatmentData['Profile Switch'].duration > 0) {
        value += " for " + treatmentData['Profile Switch'].duration + "min ";
      }
      value += Math.floor(elapsedms / 1000 / 60) + "min ago";
    }
  }
  if (treatmentData['Exercise']) {
    let elapsedms = now - Date.parse(treatmentData['Exercise'].created_at);
    if (elapsedms < hour) {
      value += "Prof: " + treatmentData['Exercise'].profile + " ";
      if (treatmentData['Exercise'].duration > 0) {
        value += " for " + treatmentData['Exercise'].duration + "min ";
      }
      value += Math.floor(elapsedms / 1000 / 60) + "min ago";
    }
  }
