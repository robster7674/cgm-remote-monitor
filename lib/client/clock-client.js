...
// graph globals - TBD move to utils
var graphWidth = 0;
var graphHeight = 0;
var graphRadius = 0;
...
function formatTimeHHMMA(d) {
  ...
  let m = d.getMinutes();
  let t;
  if (client.settings.timeFormat && (client.settings.timeFormat === 24)) {
    t = z(h) + ':' + z(m);
  } else {
...
function graphInfo () {
  ...
  const chartFactor = .1;
  let maxMGDL = client.settings.thresholds.bgTargetTop * (1 + chartFactor);
  let minMGDL = 40; // even more dynamic: client.settings.thresholds.bgTargetBottom * (1 - chartFactor);
  ...
  if (graphOverlaySGV != '1') {
    ...
    let yPct;
    yPct = (100 * (client.settings.thresholds.bgTargetTop - minMGDL) / (maxMGDL - minMGDL)) - offsetPct;
    if ((yPct >= 0) && (yPct <= 100)) {
      graphHTML += '<hr style="width: 100%; height: 0px; border: 1px dashed grey; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">';
    }
    yPct = (100 * (client.settings.thresholds.bgTargetBottom - minMGDL) / (maxMGDL - minMGDL)) - offsetPct;
    if ((yPct >= 0) && (yPct <= 100)) {
      graphHTML += '<hr style="width: 100%; height: 0px; border: 1px dashed grey; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">';
    }
    yPct = (100 * (client.settings.thresholds.bgHigh - minMGDL) / (maxMGDL - minMGDL)) - offsetPct;
    if ((yPct >= 0) && (yPct <= 100)) {
      graphHTML += '<hr style="width: 100%; height: 0px; border: 1px dotted grey; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">';
    }
    yPct = (100 * (client.settings.thresholds.bgLow - minMGDL) / (maxMGDL - minMGDL)) - offsetPct;
    if ((yPct >= 0) && (yPct <= 100)) {
      graphHTML += '<hr style="width: 100%; height: 0px; border: 1px dotted grey; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">';
    }
    ...
    graphHTML += '<span style="position: absolute; bottom: 100%; right: 0%; color: grey; font-size: 3vmin; line-height: normal;">' + maxBGText + '</span>';
    graphHTML += '<span style="position: absolute; top: 100%; right: 0%; color: grey; font-size: 3vmin; line-height: normal;">' + minBGText + '</span>';
    let d = new Date();
    d.setHours(d.getHours() - graphHours);
    graphHTML += '<span style="position: absolute; top: 100%; left: 0%; color: grey; font-size: 3vmin; line-height: normal;">' + formatTimeHHMMA(d) + '</span>';
  }
...
client.render = function render () {
  ...
  let pluginDisplayValue = clientUtils.pluginInfo();
...
client.updateParams = function updateParams () {
  ...
  for (let param in faceParams) {
    ...
    if (faceParam.substr(0, 2) === 'ow') {
      ...
      let owmUnits = owmParams[1].toUpperCase();
      let owmZip = owmParams[2].replace(/ /g,'%20');
      let owmKey = owmParams[3];
      clientUtils.setOWMParams(owmUnits, owmZip, owmKey);
    }
