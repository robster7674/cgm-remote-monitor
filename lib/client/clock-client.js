...
client.queryOWA = function queryOWA () {
  var src = 'url TBD';
  $.ajax(src, {
...
client.init = function init () {
  ...
  for (let param in faceParams) {
    ...
    if (faceParam.substr(0, 2) === 'ow') {
      owaKey = faceParam.substr(4)
      // temporarily:
      latestWeather.key = owaKey;
    }
  }
  if (owaKey !== "") {
    // setInterval(client.queryOWA, 20 * 60 * 1000); // update every 20 minutes
  }
};
