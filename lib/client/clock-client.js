...
function pluginInfo() {
  ...
  if (latestProperties.pump && latestProperties.pump.data && latestProperties.pump.data.status.display) {
...
client.updateParams = function updateParams () {
  ...
  for (let param in faceParams) {
    ...
    // not completed (threshold support lacking): :{overlaySGV bool} note: if overlaySGV is 1 then width,height are in %, otherwise pixels
