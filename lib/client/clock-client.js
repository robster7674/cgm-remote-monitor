...
function graphInfo () {
  ...
  return '<div id="graph" style="width: 100%; height: 300px; border: 1px solid black; position: relative;"> \
...
client.render = function render () {
  ...
  let graphHTMLPre = "";
  let graphHTMLPost = "";
  if (latestProperties.buckets && graphHeight > 0) {
...
client.updateParams = function updateParams () {
  ...
  for (let param in faceParams) {
    ...
    // SGV graph size params: -gr{width %}:{height %}:{radius pixels}:{overlaySGV bool}
    // example: -gr50:10:20:1
    if (faceParam.substr(0, 2) === 'gr') {
      const graphSize = faceParam.substr(2).split(":");
      graphWidth = graphSize[0];
      graphHeight = graphSize[1];
      graphRadius = graphSize[2];
      graphOverlaySGV = graphSize[3];
    }
  }
};
