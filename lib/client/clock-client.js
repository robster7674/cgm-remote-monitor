...
function treatmentsDisplay() {
  ...
  if (treatmentData['Profile Switch']) {
    ...
    if (elapsedms < hour) {
      value += "<tr><td>Prof:</td><td>" + treatmentData['Profile Switch'].profile + "</td><td>";
      if (treatmentData['Profile Switch'].duration > 0) {
        value += "for " + treatmentData['Profile Switch'].duration + "min</td><td>";
      }
      value += Math.floor(elapsedms / 1000 / 60) + "min ago</td></tr>";
    }
  }
  if (treatmentData['Exercise']) {
    let elapsedms = now - Date.parse(treatmentData['Exercise'].created_at);
    if (elapsedms < hour) {
      value += "<tr><td>Exer:</td><td>" + treatmentData['Exercise'].notes + "</td><td>";
      if (treatmentData['Exercise'].duration > 0) {
        value += " for " + treatmentData['Exercise'].duration + "min</td><td>";
      }
      value += Math.floor(elapsedms / 1000 / 60) + "min ago</td></tr>";
    }
  }
