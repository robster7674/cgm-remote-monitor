...
function pluginInfo() {
  ...
  if (latestProperties.pump && latestProperties.pump.data && latestProperties.pump.data.reservoir && latestProperties.pump.data.battery) {
  ...
  if (latestProperties.openaps && latestProperties.openaps.status) {
...
function graphInfo () {
  ...
  if (latestProperties.buckets) {
    for (let i in latestProperties.buckets) {
      if (latestProperties.buckets[i].sgvs) {
  ...
  let graphMills = graphHours * 60 * 60 * 1000;
  const maxMGDL = 500;
  const marginPct = 10;
  for (let i in accumSGV) {
    ...
    xPct = (xPct * (100.0-(2*marginPct)/100.0) + marginPct; // leave some margin since points are non-zero size
    yPct = (yPct * (100.0-(2*marginPct)/100.0) + marginPct;
    let bgColor = 'green';
    ...
    if ((xPct >= 0) && (xPct <= 100)) {
      graphHTML += '<div style="position: absolute; bottom: ' + yPct + '%; left: ' + xPct + '%; width: ' + graphRadius + 'px; height: ' + graphRadius + 'px; background-color: ' + bgColor + '; border-radius: 100%;"></div>';
    }
  }
  // add nightscout-like dashed lines at the thresholds/targets:
  let yPct;
  yPct = ((100 - (2*marginPct)) * client.settings.thresholds.bgTargetTop / maxMGDL) + marginPct;
  graphHTML += '<hr style="width: 100%; height: 1px; border-top: 1px dashed white; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">'
  yPct = ((100 - (2*marginPct)) * client.settings.thresholds.bgTargetBottom / maxMGDL) + marginPct;
  graphHTML += '<hr style="width: 100%; height: 1px; border-top: 1px dashed white; bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">'
  yPct = ((100 - (2*marginPct)) * client.settings.thresholds.bgHigh / maxMGDL) + marginPct;
  graphHTML += '<hr style="width: 100%; height: 1px; background-image: repeating-linear-gradient(to right, white, white 1px, transparent 1px, transparent 2px); bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">'
  yPct = ((100 - (2*marginPct)) * client.settings.thresholds.bgLow / maxMGDL) + marginPct;
  graphHTML += '<hr style="width: 100%; height: 1px; background-image: repeating-linear-gradient(to right, white, white 1px, transparent 1px, transparent 2px); bottom: ' + yPct + '%; margin: 0 auto; position: absolute;">'
...
client.updateParams = function updateParams () {
  ...
  for (let param in faceParams) {
    ...
    // example: -gr100:100:10:1:1
    // note: if overlaySGV is 1 then width,height are in %, otherwise pixels
