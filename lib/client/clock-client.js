...
function treatmentsDisplay() {
  ...
  if (treatmentData['Announcement']) {
    ...
    if (elapsedms < hour) {
      value += "<tr><td>Annc: " + treatmentData['Announcement'].notes + "</td><td>" + Math.floor(elapsedms / 1000 / 60) + " min ago</td></tr>";
    }
  }
  if (treatmentData['Profile Switch']) {
    let elapsedms = now - Date.parse(treatmentData['Profile Switch'].created_at);
    if (elapsedms < hour) {
      value += "<tr><td>Prof: " + treatmentData['Profile Switch'].profile;
  ...
  if (treatmentData['Exercise']) {
    ...
    if (elapsedms < hour) {
      value += "<tr><td>Exer: " + treatmentData['Exercise'].notes;
