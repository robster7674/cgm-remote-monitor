  ...
  if ($('.gt').css('font-size')) {
    ...
    if (treatmentData['Announcement']) {
      ...
      if (elapsedms < graphTime) {
        value += '<tr><td>Annc:</td><td>' + treatmentData['Announcement'].notes.replaceAll(',', ' ') + '</td><td>' + Math.floor(elapsedms / 1000 / 60) + ' min ago</td></tr>';
      }
    }
    ...
    if (treatmentData['Exercise']) {
      ...
      if (elapsedms < graphTime) {
        value += '<tr><td>Exer:</td><td>' + treatmentData['Exercise'].notes.replaceAll(',', ' ');
